...
class ConnectFourGame {
  ...
  private _getBestMove(): number {
    ...
    for (let col = 0; col < this._cols; col++) {
      ...
      moves.push({ col, val: this._minimax(this.currentPlayer * -1, 0) });
    ...
    return this._board.reduce((longestChainOnBoard, rowObj, row) => 
      rowObj.reduce((longestChainStartingInRow, _, col) => {
        if (this._board[row][col] === 0) {
          return longestChainStartingInRow;
        }
        return Math.max(longestChainStartingInRow, this._longestChainAtLocation(row, col, currentPlayer))
      }
      , longestChainOnBoard)
    , 0);
  }
  ...
  private _longestChainAtLocation(row: number, col: number, currentPlayer?: number): number {
    ...
    const playerOccupation = this._board[row][col];
    ...
    for (let direction = 0; direction < 8; direction++) {
        ...
        if (this._validAndOwned(newRow, newCol, currentPlayer != null ? currentPlayer : playerOccupation)) {
          chains[direction]++;
        }
        else {
          break;
