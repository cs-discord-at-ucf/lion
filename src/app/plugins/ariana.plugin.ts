...
import { IContainer, IMessage, ChannelType, IHttpResponse, Maybe } from '../../common/types';
...
export class ArianaPlugin extends Plugin {
  ...
  constructor(public container: IContainer) {
    ...
    this.container.httpService
      ...
      .then((res: IHttpResponse) => {
        ...
        const pagesSearch: Maybe<RegExpMatchArray> = data.match(this.rePages);

        if (!pagesSearch) {
          this.container.loggerService.warn('Failed to get Ariana page in constructor.');
          return;
        }
        const pageSearch: Maybe<RegExpMatchArray> = pagesSearch[0].match(this.rePage);
        if (pageSearch) {
  ...
  private async getAri(page: number) {
    ...
    await this.container.httpService
      ...
      .then((res: IHttpResponse) => {
        const data: string = res.data;
        const imgSearch: Maybe<RegExpMatchArray> = data.match(this.reImgs);
  ...
  public async execute(message: IMessage, args?: string[]) {
    const page = Math.floor(Math.random() * this.nPages) + 1; // valid pages 1-100
